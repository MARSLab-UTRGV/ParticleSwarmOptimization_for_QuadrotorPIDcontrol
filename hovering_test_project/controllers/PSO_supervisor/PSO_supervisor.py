"""PSO-Supervisor Controller
1. Sets as supervisor for PSO with PD/PID tuning for quadrotor
2. Params Domain: 
    {'P': [0, 12]}
    {'I': [0, 12]}
    {'D': [0, 12]}
...
4. Reset hover_exp_PSO_roll Controller based on failing conditions
...
def measAvgStdDevforParticles(particles):
    ...
    optim_dir = os.path.normpath(os.path.join(parent_file + "\hover_exp_PSO_roll"))
...
def createAndCollectParticles(params_file, filedir, num_of_particles):
    ...
    for i in range(num_of_particles):
        ...
        if os.path.isfile(optim_file):
            ...
            #init_dict["pitch_Kd"] = np.random.uniform(0, 2)
            #init_dict["roll_Kp"] = np.random.uniform(0, 2)
...
def writeMavicState(input_arr, filename):
    ...
    csv_dict = {'x_pos': input_arr[0],
                ...
                'front_left_motor': input_arr[13],
                'front_right_motor': input_arr[14],
                'rear_left_motor': input_arr[15],
                'rear_right_motor': input_arr[16]}
...
def updateParticleCSV(new_params, params, filedir, idx):
    ...
    #params["pitch_Kp"] = new_params[3]
    #params["pitch_Ki"] = new_params[4]
    #params["pitch_Kd"] = new_params[5]
    #params["throttle_Kp"] = new_params[0]
    params["throttle_Ki"] = new_params[0]
    #params["throttle_Kd"] = new_params[0]
...
def createPBestParamsFit(particles):
    ...
    for i, particle_dict in enumerate(particles):
        pbest_dict[i + 1] = (particle_dict['throttle_Ki'], init_fitness)
    return pbest_dict
...
def checkPBestFitness(params, param_fitness, pbest_dict, dict_idx):
    ...
    if param_fitness < pbest_params[1]:
        pbest_dict[dict_idx] = (params['throttle_Ki'], param_fitness)
...
def getFitnessFromTrialsITAE(trials, trials_times, calcd_time_step=0.032, feat_exp=0.2, time_exp=3.0):
    fitness_avg_arr = []
    print("# of trials: {}".format(len(trials)))
    for i, trial in enumerate(trials):
        exp_error = 0.25#add as parameter for various PID outputs
...
#overshoot is collect from the average of overshoot,
#when drone falls above or below expected
def getFitnessFromTrials(trials, trials_times, feat_exp=0.2, time_exp=0.3):
    overshoot_avg =  []
    timestep_avg = []
    for i, trial in enumerate(trials):
        overshoot_arr = []
        ...
        for j in range(len(trial)):
            if trial[j] > feat_exp or trial[j] < -feat_exp:
                timestep_arr.append(trial_time[j])
                overshoot_arr.append(trial[j])
                while(trial[j] > feat_exp or trial[j] < -feat_exp):
                    timestep_arr.append(trial_time[j])
                    overshoot_arr.append(trial[j])
                    j += 1
                    if j == len(trial):
        ...
        timestep_avg.append(timestep_arr)

    #fix this part to include full fitness function for overshoot and time
    time_act = 0
    time_act_avg = 0
    ovsht_sum = 0
    ovsht_avg = 0
    for i, overshoot in enumerate(overshoot_avg):
        if not overshoot:
            print("found no overshoot")
            ovsht_sum += 10
            time_act = 10 ###change where fitness is not 0
        else:
            ovsht_sum += np.mean(overshoot)
            timespan = timestep_avg[i]
            time_act += timespan[len(timespan) - 1] - timespan[0]
        
        print("ovsht_sum: {:.4f}".format(ovsht_sum))
        ovsht_avg = ovsht_sum / len(overshoot_avg)
        time_act_avg = time_act / len(overshoot_avg)
        print("avg-feat overshoot: {:.4f}".format(ovsht_avg))
        print("avg-time: {:.4f}".format(time_act_avg))
        ovsht_error = abs(ovsht_avg - feat_exp)
        print("ovsht_error: {:.4f}".format(ovsht_error))

    fitness = math.log10((abs(ovsht_avg)/feat_exp) + 1) + math.log10((abs(time_act_avg)/time_exp) + 1)
    print("fitness: {:.4f}".format(fitness))
    return fitness


def collectFitnessScore(files_dict, params, calcd_time_step=0.032):
    ...
    #roll_fitness = getFitnessFromTrials(roll_trials,
    #                                    roll_times,
    #                                    feat_exp=1e-5,
    #                                    time_exp=3.2)
    ...
    #pitch_fitness = getFitnessFromTrials(pitch_trails,
    #                                     pitch_times,
    #                                     feat_exp=0.02,
    #                                     time_exp=0.3)
...
class CrazyFlieSuper(Supervisor):
    ...
    def run(self, params, files_dict, waypoints):
        ...
        print("using time step: {}".format(self.time_step / 1000))
        calcd_time_step = self.time_step / 1000

        sim_steps = np.arange(0, 30, calcd_time_step)
        ...
        updateOptimFile(params, files_dict=files_dict)
        ...
        #perform PSO with given overshoots
        for i in range(5):
            ...
            while self.step(self.time_step) != -1:
                if self.super_rcvr.getQueueLength() > 0:
                    ...
                    self.super_rcvr.nextPacket()
                    acc_msg = self.super_rcvr.getFloats()
                    #print("acc msg4: {}".format(acc_msg))
                    self.super_rcvr.nextPacket()
                    time_msg = self.super_rcvr.getFloats()
                    #print("time msg5: {}".format(time_msg))
                    self.super_rcvr.nextPacket()
                    front_mtr_msg = self.super_rcvr.getFloats()
                    #print("front-motors msg6: {}".format(front_mtr_msg))
                    self.super_rcvr.nextPacket()
                    rear_mtr_msg = self.super_rcvr.getFloats()
                    #print("rear-motors msg7: {}".format(rear_mtr_msg))
                    self.super_rcvr.nextPacket()
                    ...
                    mavic_state = [pos_msg[0], pos_msg[1], pos_msg[2],#(x,y,z)
                                    ...
                                    acc_msg[0], acc_msg[1], acc_msg[2],#(roll_acc, pitch_acc, yaw_acc)
                                    time_msg[0], #timestep
                    ...
                    #print("posPID msg8: {}".format(posPID_msg))
                    self.super_rcvr.nextPacket()
                    rpPID_msg = self.super_rcvr.getFloats()
                    #print("roll-pitchPID msg9: {}".format(rpPID_msg))
                    self.super_rcvr.nextPacket()
                    ytPID_msg = self.super_rcvr.getFloats()
                    #print("yaw-throttlePID msg10: {}".format(ytPID_msg))
                    self.super_rcvr.nextPacket()
                    rpInput_msg = self.super_rcvr.getFloats()
                    #print("roll-pitch inputs msg11: {}".format(rpInput_msg))
                    self.super_rcvr.nextPacket()
                    ytInput_msg = self.super_rcvr.getFloats()
                    #print("yaw-throttle inputs msg12: {}".format(ytInput_msg))
                    self.super_rcvr.nextPacket()
                    diff_alt_msg = self.super_rcvr.getFloats()
                    #print("diff-alt msg13: {}".format(diff_alt_msg))
                    self.super_rcvr.nextPacket()
                    ...
                    if (position_cond[0] > waypoints[0] + wypnt_tolerance) or (position_cond[0] < waypoints[0] - wypnt_tolerance):
                        ...
                        position_bool = True
                        print("y became true")
                    elif (position_cond[2] > waypoints[2] + (wypnt_tolerance * float(5))):#for altitude
                        position_bool = True
                        print("z became true")
        ...
        fitness = collectFitnessScore(files_dict, params, calcd_time_step)
...
def main():
    ...
    param_filedir = r"C:\Users\ericx\OneDrive\Desktop\mine\grad_school\MARS_Lab_Swarm\Gazebo_dragonFly\webots_tutorial\hovering_test_project\controllers\hover_exp_PSO_roll"
    ...
    state_filename = state_filedir + r"\mavic_state3.csv"
    pid_filename = state_filedir + r"\PID_and_inputs3.csv"
    ...
    for key, val in pbest_dict.items():
        print("part-{}-->{}".format(key, val[0]))
    gbest = pbest_dict[1]
    print("init-gbest: {}".format(gbest[0]))
    
    
    #V-particle velocity
    V_roll = np.zeros(1)
    print("initialized vroll: {}".format(V_roll))
    ...
    for i in range(iter_num):
        ...
        for j, param in enumerate(particles):
            print("p{}: {}".format(j, param['throttle_Ki']))
        ...
        for idx, params in enumerate(particles):
            print("running particle-{} with params: {}".format(idx+1, params['throttle_Ki']))
            print("pbest: {}---{}".format(pbest_dict[idx+1][0], pbest_dict[idx+1][1]))
            #create waypoint for hovering, later-->implement collectWaypoints from CSV
            waypoints = np.array([0.0001, 0.0001, float(params["target_altitude"])])
            print("using waypoints: (x: {}, y: {}, z: {} )".format(waypoints[0], waypoints[1], waypoints[2]))
            ...
            gbest_arr = np.array([float(gbest_params["throttle_Ki"])])

            params_arr = np.array([float(params["throttle_Ki"])])

            print("pbest:\n{}".format(pbest_arr))
            print("gbest:\n{}".format(gbest_arr))
            print("params:\n{}".format(params_arr))

            first_term = np.multiply(wgt, V_roll)
            second_term_a = c1*r1
            ...
            V_roll = np.add(np.add(first_term, second_term), third_term)
            print("V_roll: {}".format(V_roll))
            new_params = params_arr + V_roll
