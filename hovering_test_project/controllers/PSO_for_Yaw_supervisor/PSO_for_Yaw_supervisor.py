"""PSO for Yaw-Supervisor Controller
1. Supervisor for PSO with PID tuning for Yaw tuning
2. Params Domain: 
    {'P': [0, 1]}
    {'I': [0, 1]}
    {'D': [0, 1]}
...
class CrazyFlieSuper(Supervisor):
    ...
    def run(self, params, files_dict, waypoints):
        ...
        for i in range(5):
            ...
            while self.step(self.time_step) != -1:
                if self.super_rcvr.getQueueLength() > 0:
                    ...
                    #print("time msg6: {}".format(bot_state_msg))
                    self.super_rcvr.nextPacket()
                    front_mtr_msg = self.super_rcvr.getFloats()
                    #print("front-motors msg7: {}".format(front_mtr_msg))
                    self.super_rcvr.nextPacket()
                    rear_mtr_msg = self.super_rcvr.getFloats()
                    #print("rear-motors msg8: {}".format(rear_mtr_msg))
                    self.super_rcvr.nextPacket()
                    ...
                    position_bool = False
                    if (position_cond[0] > self.curr_waypnt[0] + wypnt_tolerance) or (position_cond[0] < self.curr_waypnt[0] - wypnt_tolerance):
                        position_bool = True
                        print("x became true")
                    elif (position_cond[1] > self.curr_waypnt[1] + wypnt_tolerance) or (position_cond[1] < self.curr_waypnt[1] - wypnt_tolerance):
                        position_bool = True
                        print("y became true")
                    elif (position_cond[2] > self.curr_waypnt[2] + (wypnt_tolerance * float(5))):#for altitude
                        position_bool = True
                        print("z became true")
        ...
        try:
            fitness_feats = [0, 0, yaw_feat_exp, params['target_altitude']]
            print("using calcd feats...")
        except:
            fitness_feats = [0, 0, 0, params['target_altitude']]
            print("using defult feats")
...
def checkPBestFitness(params, param_fitness, pbest_dict, dict_idx):
    ...
    if param_fitness < pbest_params[1]:
        pbest_dict[dict_idx] = (params['yaw_Kp'], param_fitness)
...
def createPBestParamsFit(particles):
    ...
    for i, particle_dict in enumerate(particles):
        pbest_dict[i + 1] = (particle_dict['yaw_Kp'], init_fitness)
    return pbest_dict
...
def measAvgStdDevforParticles(particles):
    ...
    optim_dir = os.path.normpath(os.path.join(parent_file + "\hover_yaw_exp_PSO"))
...
def main():
    ...
    param_filedir = r"C:\Users\ericx\OneDrive\Desktop\mine\grad_school\MARS_Lab_Swarm\Gazebo_dragonFly\webots_tutorial\hovering_test_project\controllers\hover_yaw_exp_PSO"
    ...
    for i in range(iter_num):
        ...
        for idx, params in enumerate(particles):
            ...
            print("running particle-{} with params: {}".format(idx+1, params['yaw_Kp']))
            ...
            gbest_arr = np.array([float(gbest_params["yaw_Kp"])])

            params_arr = np.array([float(params["yaw_Kp"])])
    ...
    print("starting yaw chg experiment...")
