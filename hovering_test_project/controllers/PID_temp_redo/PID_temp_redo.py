...
class Mavic(Supervisor):
    ...
    def __init__(self, waypoints, time_step, params, takeoff_threshold_velocity=1):
        ...
        #arrays to collect dot-euler-angels from rotation-matrix
        self.phi_dot_arr = np.full((1, 40), float(0))[0]
        self.theta_dot_arr = np.full((1, 40), float(0))[0]
        self.psi_dot_arr = np.full((1, 40), float(0))[0]
        self.euler_dot_idx = 0
        #arrays to collet trans.-dot velocity
        self.xdot_global_arr = np.full((1, 40), float(0))[0]
        self.ydot_global_arr = np.full((1, 40), float(0))[0]
        self.zdot_global_arr = np.full((1, 40), float(0))[0]
        self.trans_dot_idx = 0
    ...
    #return the euler kinematic equations
    def getEulerKinematicEqs(self, roll_ang, pitch_ang):
    ...
    def getLocalToGlobalRotation(self, roll_ang, pitch_ang, yaw_ang):
    ...
    #update trans-dot arrays per time step
    def updateTransDotArrays(self, trans_dot_velocities):
        if self.trans_dot_idx == 40:
            self.rollTransDotArrays()
            self.xdot_global_arr[-1] = trans_dot_velocities[0]
            #print("xdot_global_arr: {}".format(self.xdot_global_arr))
            self.ydot_global_arr[-1] = trans_dot_velocities[1]
            #print("ydot_global_arr: {}".format(self.ydot_global_arr))
            self.zdot_global_arr[-1] = trans_dot_velocities[2]
        else:
            self.xdot_global_arr[self.trans_dot_idx] = trans_dot_velocities[0]
            #print("xdot_global_arr: {}".format(self.xdot_global_arr))
            self.ydot_global_arr[self.trans_dot_idx] = trans_dot_velocities[1]
            #print("ydot_global_arr: {}".format(self.ydot_global_arr))
            self.zdot_global_arr[self.trans_dot_idx] = trans_dot_velocities[2]
            #print("zdot_global_arr: {}".format(self.zdot_global_arr))
            self.trans_dot_idx += 1
    

    #if acc_idx == size of arays, 
    #then-->roll the arrays to left and write val to last index 
    def rollTransDotArrays(self):
        self.xdot_global_arr = np.roll(self.xdot_global_arr, -1)
        self.ydot_global_arr = np.roll(self.ydot_global_arr, -1)
        self.zdot_global_arr = np.roll(self.zdot_global_arr, -1)

    
    #update euler-dot arrays per timestep
    def updateEulerArrays(self, euler_dot_angles):
        if self.euler_dot_idx == 40:
            self.euler_dot_idx = 0
        self.phi_dot_arr[self.euler_dot_idx] = euler_dot_angles[0]
        #print("roll_dot_arr: {}".format(self.roll_dot_arr))
        self.theta_dot_arr[self.euler_dot_idx] = euler_dot_angles[1]
        #print("pitch_dot_arr: {}".format(self.pitch_dot_arr))
        self.psi_dot_arr[self.euler_dot_idx] = euler_dot_angles[2]
        #print("yaw_dot_arr: {}".format(self.yaw_dot_arr))
        self.euler_dot_idx += 1
    ...
    def run(self, files_dict, waypoints, params):
        ...
        while self.step(self.time_step) != -1 and takeoff_bool:
            ...
            euler_kin_matrix = self.getEulerKinematicEqs(roll_ang, pitch_ang)
            #print("euler_kin_matrix:\n{}".format(euler_kin_matrix))
            
            #convert local angular-rates fm body to global eular-rates
            ang_vel = np.array([roll_vel, pitch_vel, yaw_vel])
            euler_rates = np.matmul(euler_kin_matrix, ang_vel)
            #update euler-dot arrays
            self.updateEulerArrays(euler_rates)
            #print(euler_rates)-->first-order time derivative in global frame(3, )
            phi_dot = euler_rates[0]
            theta_dot = euler_rates[1]
            psi_dot = euler_rates[2]
            
            #use accel. meas. fm local-acc rates to get global position estimation
            xvel_local = -integrate.simpson(self.xacc_arr, self.time_interval_arr)
            #if (self.getTime() * 1000) % 100 == 0:
            #    print("xvel_local: {}".format(xvel_local))
            #print("xvel_local: {}".format(xvel_local))
            yvel_local = -integrate.simpson(self.yacc_arr, self.time_interval_arr)
            #if (self.getTime() * 1000) % 100 == 0:
            #    print("yvel_local: {}".format(yvel_local))
            #print("yvel_local: {}".format(yvel_local))
            zvel_local = integrate.simpson(np.subtract(self.altacc_arr, 9.81), self.time_interval_arr)
            #print("alt-vel_local: {}".format(zvel_local))
            #translate translational local vel. into global frame trans. vel.
            trans_vel_local = np.array([xvel_local, yvel_local, zvel_local])
            

            #collect trans-dot(xdot, ydot, zdot)
            trans_dot_global = np.matmul(local2global_rot_matrix, trans_vel_local)
            #print(trans_dot_global)
            #update trans-dot arrays
            self.updateTransDotArrays(trans_dot_global)
            xdot_global = trans_dot_global[0]
            #print("xdot_global: {}".format(xdot_global))
            ydot_global = trans_dot_global[1]
            #print("ydot_global: {}".format(ydot_global))
            zdot_global  = trans_dot_global[2]
            #print("zdot_global: {}".format(zdot_global))
            ...
            #collect inputs
            #roll
            roll_input = float(params["k_roll_p"]) * clamp(roll, -1, 1) + roll_vel + self.rollPID(roll_ang) + clamp(self.yposPD(y_pos), -1, 1)
            #roll_input = self.rollDotPID(roll_acceleration)
            #roll_input = float(params["k_roll_p"]) * clamp(roll, -1, 1) + roll_vel + self.rollPID(roll_ang)
            #pitch
            pitch_input = float(params["k_pitch_p"]) * clamp(pitch, -1, 1) + pitch_vel + self.pitchPID(pitch_ang) + clamp(self.xposPD(-x_pos), -1, 1)
            #pitch_input = self.pitchDotPID(pitch_acceleration)
            #pitch_input = float(params["k_pitch_p"]) * clamp(pitch, -1, 1) + pitch_vel + self.pitchPID(pitch_ang)
            #yaw
            ...
            #vertical_input = (1 / (np.cos(roll) * np.cos(pitch))) * (self.throttlePID(altitude) + (float(quad_mass) * float(world_grav)))
...
def writeMavicState(input_arr, filename):
    ...
    csv_dict = {'x_pos': input_arr[0],
                ...
                'phi_dot': input_arr[16],
                'theta_dot': input_arr[17],
                'psi_dot': input_arr[18],
                'xvel_local': input_arr[19],
                'yvel_local': input_arr[20],
                'zvel_local': input_arr[21],
                'xdot_global': input_arr[22],
                'ydot_global': input_arr[23],
                'zdot_global': input_arr[24],
                'xpos_global': input_arr[25], 
                'ypos_global': input_arr[26],
                'zpos_global': input_arr[27],
                'roll_ang': input_arr[28],
                'pitch_ang': input_arr[29],
                'yaw_ang': input_arr[30]}
